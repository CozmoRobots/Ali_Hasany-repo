...
def prob_based_on_cc(card_count):
    ...
    if big_cards > small_cards:
        return [0.8, big_cards, small_cards]
    else:
        return [0.2, big_cards, small_cards]
...
def play_hand(dealer_hand, player_hands, curr_player_results, dealer_cards, hit_stay, card_count, dealer_bust, players):
    ...
    return curr_player_results, dealer_cards, card_count, dealer_bust, big_cards, small_cards
...
for sim in range(simulations):
    ...
    while len(dealer_cards) > 20:
        ...
        curr_player_results, dealer_cards, card_count, dealer_bust, big_card, small_card = play_hand(dealer_hand, player_hands, curr_player_results, dealer_cards, hit_stay, card_count, dealer_bust, players)
        ...
        #track card count
        big_card_count.append(big_card)
        small_card_count.append(small_card)
...
model_df['big_card_count'] = big_card_count
model_df['small_card_count'] = small_card_count
...
attribute_names = ['num_of_player', 'num_of_decks','big_card_count','small_card_count', 'dealer_card', 'dealer_value', 'dealer_bust', 'init_hand', 'hit', 'result', 'outcome']
...
for index in range(players):
    ...
    player_df = step5[['big_card_count','small_card_count','dealer_card', 'dealer_value', 'dealer_bust', att1, att2, att3, att4]]
...
final_df.to_csv(f'ali_{players}_{num_decks}.csv', index=False)
